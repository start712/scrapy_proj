...
class data_cleaner(object):
    ...
    def get_data(self, length = 100):
        ...
        data1 = mysql_connecter.connect(sql1, dbname='spider', ip='116.62.230.38', user='spider', password='startspider')
        data1 = self.standardizing(data1)
        sql2 = r"SELECT `key`, `detail`, `status`, `fixture_date`, `url` FROM `monitor` WHERE `status` = 'sold'" # LIMIT %s [length,],
        data2 = mysql_connecter.connect(sql2, dbname='spider', ip='116.62.230.38', user='spider', password='startspider')
    ...
    def duplicate_check(self, onselldata, sold_data):
        pass
    ...
    def main(self):
        onsell_data, sold_data = self.get_data()
        data = self.duplicate_check(onsell_data, sold_data)
